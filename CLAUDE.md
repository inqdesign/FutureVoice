...
- **Exception 1: SETTINGS speaks the learner's own language** (2026-08-17). `MeTab` carries its own `.environment(\.locale, Locale(identifier: appState.nativeLanguage))`, and that covers everything pushed or presented from it — the plan page, `UsageDetailView`, `CreditGuideView`, `PublicIntroView`, `InviteView`, `AddLanguageSheet`, the privacy page, every alert. Free exposure is only free on words recognized by shape beside an icon; nothing in the drawer is that. Every row asks the learner to decide, consent, or destroy something, and the page that *sets the app's language* cannot be the one page they can't read. `AccountStatus`, `UsageBreakdown` and `AppAppearance.label` therefore build their labels with `explain(…)`, not `chrome(…)` — they are rendered nowhere else. Same `String`-typed trap as onboarding, and it is exactly how this broke: `MeTab.row(icon:title:subtitle:)` takes `String`, so ~35 row titles were bare literals that never localized at all, in either language. A screenshot harness case that renders a settings subpage must set the same locale by hand or it shoots a screen no learner sees (`DebugCaptureHarness`, `"usage"`).
- **Exception 2: onboarding speaks the learner's own language** (`UILanguage.isOnboarding`, true until setup + the voice clone + the daily-call intro are all done). On the first two screens no target has been picked yet, so `targetLanguage` is still its `"en"` placeholder and a Korean phone opened the app in English; and nothing in the flow is recognized-by-shape anyway — every screen asks the learner to decide, consent, or follow a recording instruction. So the whole flow follows `LanguageCatalog.currentNative`, which defaults to the DEVICE language. Chrome-follows-target resumes on the tabs. That means an onboarding string in a `String` (a computed nav title, a `switch` returning a label) still has to go through `chrome(…)`/`explain(…)` — a bare literal there is frozen English, which is how most of onboarding shipped untranslated until 2026-08-16.
