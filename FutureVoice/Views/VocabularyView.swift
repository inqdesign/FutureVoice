...
/// EVERY field decodes leniently — a missing key is an empty value, never a
/// thrown error. This is not defensiveness for its own sake: the entry is
/// written by a model and cached FOREVER, and both lookup paths swallow their
/// decode error (`try?` on the table read, `catch` on the function), so one
/// absent key made the card say "couldn't load it" for that word permanently —
/// retry re-decoded the same row and failed identically. The expression prompt
/// even asks for `"pos": ""` inside a sense, which a model reasonably omits.
struct WordEntry: Codable, Equatable {
    struct Sense: Codable, Equatable, Identifiable {
        ...
        enum CodingKeys: String, CodingKey { case pos, meaning, note }
        init(pos: String, meaning: String, note: String?) {
            self.pos = pos; self.meaning = meaning; self.note = note
        }
    ...
    struct Phrase: Codable, Equatable, Identifiable {
        ...
        enum CodingKeys: String, CodingKey { case phrase, meaning }
        init(phrase: String, meaning: String) {
            self.phrase = phrase; self.meaning = meaning
        }
        init(from decoder: Decoder) throws {
            let c = try decoder.container(keyedBy: CodingKeys.self)
    ...
    struct Example: Codable, Equatable, Identifiable {
        ...
        enum CodingKeys: String, CodingKey { case text, meaning }
        init(text: String, meaning: String?) {
            self.text = text; self.meaning = meaning
        }
        init(from decoder: Decoder) throws {
            let c = try decoder.container(keyedBy: CodingKeys.self)
    ...
    enum CodingKeys: String, CodingKey { case pos, senses, examples, phrases, properNoun }
    init(pos: String?, senses: [Sense], examples: [Example],
         phrases: [Phrase], properNoun: String?) {
        self.pos = pos; self.senses = senses; self.examples = examples
        self.phrases = phrases; self.properNoun = properNoun
    }
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        pos = try c.decodeIfPresent(String.self, forKey: .pos)
        // A sense whose own decode fails drops out rather than taking the
        // whole entry with it — three good senses beat "couldn't load it".
        senses = (try c.decodeIfPresent([Sense].self, forKey: .senses)) ?? []
        examples = (try c.decodeIfPresent([Example].self, forKey: .examples)) ?? []
        phrases = (try c.decodeIfPresent([Phrase].self, forKey: .phrases)) ?? []
        properNoun = try c.decodeIfPresent(String.self, forKey: .properNoun)
    }
}
...
enum WordLore {
    ...
    static func entry(for word: String, native: String,
        ...
        do {
            let rows: [EntryRow] = try await SupabaseProvider.shared.from("word_entry")
            ...
            // A DECODE failure here looks exactly like a cache miss, and the
            // regenerated entry lands in the same undecodable shape — so the
            // word fails forever, silently. Say so at least in DEBUG.
            #if DEBUG
            print("WORDLORE cache read failed for \"\(word)\": \(error)")
            #endif
        }
        ...
        do {
            ...
            #if DEBUG
            print("WORDLORE lookup failed for \"\(word)\": \(error)")
            #endif
            return nil
        }
    }
