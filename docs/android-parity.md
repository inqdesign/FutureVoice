...
- [x] 네 탭을 iOS 스크린샷에 맞춤 (2026-09-14) — 시뮬레이터 캡처 하네스(`-capture home|watchtab|
      practice-studying|progress`)로 iOS를 찍어 놓고 하나씩 비교했다. **Talk**: 지구본+언어 코드 칩,
      가운데 연속 칩(0이면 "활동"), 아바타의 사용량 링, 큰 인사말, 첫 대화 안내 카드.
      **Watch**: 픽셀 큰 제목, "사람" 헤더와 점선 만들기 버블, 상황 카드(재생 글리프·상대·상대 시각),
      있을 법한 상황 2열. **Practice**: 검은 선택 칩+개수, 완료 알약, 타일마다 제 아이콘과
      "학습할 것 N ›"(`StudyCollections`가 그 페이지가 나열하는 것을 센다), 책 카드는 세로 해부학
      (아이콘·점수 링·마지막 학습·마스터리 줄). **Progress**: 레벨 카드(추정 레벨·무엇을 할 수 있는지),
      영역별은 0–100 점수 대신 밴드.
