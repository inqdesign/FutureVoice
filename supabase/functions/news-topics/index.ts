// Platform news pool.
//
// Body: { categories: string[], language?: string }
...
Deno.serve(async (req) => {
  ...
  let body: { categories?: unknown; language?: unknown }
  ...
  for (const row of rows ?? []) {
    for (const t of row.topics as { title: string; blurb: string }[]) {
      if (t?.title) out.push({ category: row.category, title: t.title, blurb: t.blurb ?? "" })
    }
  }
