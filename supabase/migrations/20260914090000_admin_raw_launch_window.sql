-- The console is about the LAUNCH now, not the beta.
--
-- Everything on the page was windowed from 2026-08-10 — the day Talk became
-- the unit of account — which was right while the only users were six beta
-- testers and the owner. The app went on the App Store on 2026-09-12, and
-- against 17 new signups the beta month is noise: the owner alone carries 630
-- turns against the launch week's 215, so every average, every streak lane and
-- every retention row was mostly one person practising in August.
--
-- `w.start` therefore moves to 2026-09-12 and drives ACTIVITY only — cells,
-- scenes, sessions, errors, feature use. The two cost windows deliberately do
-- NOT move:
--   * `cost_start` (2026-08-14) is where per-scene cost stops being inflated
--     by scene audio whose plays were never recorded. Unit economics are a
--     property of the product, not of the launch.
--   * `cost_history` (2026-08-10) keeps the spend chart and the per-user cost
--     totals whole. What an account has cost us is a cumulative fact, and a
--     burn chart that starts on launch day cannot be compared with anything.
--
-- Also: `reviews` gains its TIMESTAMP. The page now draws one chronological
-- feed of everything that happened — signup, clone, first talk, trial, cancel,
-- review — and a date-only row could not be placed in it.
