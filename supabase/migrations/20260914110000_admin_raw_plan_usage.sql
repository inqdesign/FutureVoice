-- Per-account plan usage, counted the way the SERVER counts it.
--
-- The console could say what a plan costs and what an account has cost us,
-- and could not say how much of the plan that account has actually spent —
-- so "is this person about to hit the wall" and "is this plan the wrong size
-- for them" had no answer on the page. It also never printed a renewal date,
-- which is the one date the beta comps and every trial are waiting on.
--
-- `plan_usage` replicates `talk_allowance()` and `begin_scene_play()` rather
-- than approximating them, so the number on the page is the number the wall
-- is made of:
--   * the period is `billing_period_start(user)` — the LATEST
--     `current_period_start` of an entitled row, not the calendar month;
--   * talk seconds are `tts_char_pool.chars` for `talk_seconds` and
--     `scene_seconds` (the column is misnamed; it holds seconds for those two
--     actions), from that date;
--   * scenes are rows in `scene_plays` from that date;
--   * a TRIAL is pro-rated 7/30 off the LIGHT tier's figure, whatever plan is
--     being trialled — a 7-day Plus trial cannot spend a month of Plus;
--   * `talk_unlimited` suppresses the talk cap, and only outside a trial.
--
-- Rows are returned for every account with an entitled subscription
-- (trialing · active · grace), including comps: a comp has a period and a
-- cap like any other plan, and the five beta seats ending 2026-09-21 are
-- exactly what this was asked for.
