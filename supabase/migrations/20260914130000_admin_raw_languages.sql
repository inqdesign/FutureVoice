-- Which language is this person learning — from every place that knows.
--
-- `langs` read `talk_seconds_by_language` alone, which is written only by
-- `charge_talk_seconds(p_language)` — i.e. only when someone has actually
-- SPOKEN. So a learner who published a German persona, or watched a German
-- scene, or enrolled and has not talked yet, had no language at all and the
-- page showed them blank. Reported 2026-09-14 as "영어만 표시되는 것 같다".
--
-- `profiles.target_language` is NOT the answer, tempting as it looks: all 31
-- rows say en/ko/b1 because nothing in the app has ever written it (the one
-- `target_language` in the client is a PostHog property). A column of
-- defaults would have made every learner English by construction, which is
-- the bug it looks like a fix for.
--
-- So: the union of the three places that only ever record something real —
--   talk    `talk_seconds_by_language`      they spoke it (carries seconds)
--   ledger  `usage_ledger.metadata.language` they spent something on it
--   persona `public_personas.language`       they published an intro in it
-- per user AND per language, with the seconds and which sources agree. A
-- language present in none of them is a language nobody has used, and the
-- page says "기록 없음" rather than quietly printing English.
