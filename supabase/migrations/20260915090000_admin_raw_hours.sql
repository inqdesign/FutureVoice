-- When do people actually talk?
--
-- Nothing on the page had an hour in it: the feed showed clock times one
-- event at a time, and every chart was bucketed by DAY. So "is this a
-- morning-commute product or a before-bed product" — which decides what time
-- the daily call should ring, and when a push is worth sending — had no
-- answer.
--
-- `hours` buckets metered talk seconds by UTC hour-of-day, per user. UTC on
-- purpose: the client rotates it to whichever zone is being read (KST is
-- UTC+9 with no DST, so that rotation is exact), and keeping the offset out
-- of the database means the page can offer more than one without a migration.
-- Per user as well as per hour, so a person's own usual time can be shown on
-- their row rather than only the crowd's.
--
-- `talk_time` rather than turn calls: it is the metered second, the same
-- quantity the ring and the allowance are made of, and it exists for every
-- talking minute rather than once per reply.
...
select jsonb_build_object(
  ...
  'hours', (select coalesce(jsonb_agg(jsonb_build_object(
                     'id', user_id::text, 'h', h,
                     'secs', secs, 'ticks', ticks, 'days', days)), '[]'::jsonb)
              from (select user_id,
