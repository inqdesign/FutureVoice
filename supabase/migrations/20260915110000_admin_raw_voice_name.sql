-- The clone's NAME and voice id, beside the person.
--
-- `users_raw` carried `clone` (a boolean) and `clone_at`, which answers
-- "did they get through onboarding" and nothing else. `voice_clones` has
-- held `name` since the clone function first wrote it — "Future 예원",
-- "Future young" — and `elevenlabs_voice_id`, which is the only handle that
-- matches a row here to a voice in the ElevenLabs dashboard, to
-- `scripts/eleven-history.sh`, or to a TTS bill.
--
-- The lateral for `clone_at` keeps its ORDER: oldest first, because that is
-- when the learner first cloned. The name and the id come from the ACTIVE
-- row instead (a remix makes a new clone and deactivates the old), falling
-- back to the newest when nothing is flagged active.
